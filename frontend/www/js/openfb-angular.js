...
angular.module('comdrsmoothieappApp')
    ...
    .factory('OpenFB', function ($rootScope, $q, $window, $http) {
         ...
         */
        var init = function(appId, store, redirectURL) {
            if (!initialized) {
                fbAppId = appId;
        ...
        function login(fbScope, successCallbk, failureCallbk) {

            successCallback = successCallbk;
            failureCallback = failureCallbk;
        ...
        function shareCallback(url) {
            ...
            if (url.indexOf("post_id=") > 0) {
                alert ("Success!");
            } else if (url.indexOf("error_code=") > 0) {
